...
import { ConvexHandlerError, fetchTypedConvex } from "../../../Fetch";
import { ConvexSettingsAPIProps } from "../../../Types";
...
export const httpActionAuthUserCreate = httpAction(async ({ runMutation }, req) => {
  ...
  if (type === "user.created") {
    ...
    let [err, _id] = await fetchTypedConvex(runMutation(internal.auth.mutation.create, {
    ...
    if (!err) {
      /** @description Create the unique settings object for the user */
      [err] = await fetchTypedConvex(runMutation(internal.sync.settings.mutation.create, {
        userId: _id,
        ...getDefaultSettingsObject(_id)
      }));
    }
